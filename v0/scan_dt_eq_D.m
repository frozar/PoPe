...
case 0 % custom
  ...
  %----------------------------------
  Nx_range = 1024; %[16 32 64 128 256 512 1024]; % number of points in space
  dt_range = 0.01*2.^[0:-1:-3]; % time step for computation
  dt_diag_range = 0.2; % time step for diagnostics
  ...
  % initial conditions
  %----------------------------------
  init_f = 3; % choice of the shape of f_init
...
tmp(1,:)=abs(tmp(1,:)-c);
tmp(2,:)=abs(tmp(2,:)-D);
error_1_c = mean(tmp(1,:));
error_1_D = mean(tmp(2,:));
[error_1_c error_1_D]
...
subplot(2,2,1)
plot(dt,D_eff-D,'*','color',colors(iter_color,:));hold on;
axis square

subplot(2,2,2)
plot(log10(dt),log10(abs(D_eff-D)),'*','color',colors(iter_color,:));hold on;
axis square

subplot(2,2,3)
plot(dt,error_1_D,'*','color',colors(iter_color,:));hold on;
axis square

subplot(2,2,4)
plot(log10(dt),log10(error_1_D),'*','color',colors(iter_color,:));hold on;
axis square
