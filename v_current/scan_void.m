...
case 0 % custom
  ...
  % physical parameters
  %----------------------------------
  c_range = 0.1; % advection coefficient
  D_range = 0.0; % diffusion coefficient
  T_range = 100; % duration of simulation
  Lx_range = 4*2*pi; % size of the box
  
  %----------------------------------
  % numerical parameters
  %----------------------------------
  Nx_range = [128]; % number of points in space
  dt_range = 0.1*[1]; % time step for computation
  dt_diag_range = 1; % time step for diagnostics
  choice_derivative_range = [0]; %[1 2 3 0]; % centered derivatives using a stencil of 1 + 2 x choice_derivative points
  % 0 = fft
